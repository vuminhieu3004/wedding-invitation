...
export const guest = (() => {
    ...
    const slide = async () => {
         ...
         * @returns {Promise<void>}
         */
        const runSlide = async (slides, host, activeClass) => {
            if (!slides || slides.length === 0 || !host) {
                return;
            }

            host.dispatchEvent(new Event('undangan.slide.stop'));
            if (window.getComputedStyle(host).display === 'none') {
                return;
            }
            ...
            host.addEventListener('undangan.slide.stop', () => {
                run = false;
            });
        ...
        const desktopSlides = Array.from(document.querySelectorAll('.slide-desktop'));
        await runSlide(desktopSlides, desktopEl, 'slide-desktop-active');

        const mobileEl = document.getElementById('home');
        const mobileSlides = Array.from(document.querySelectorAll('.slide-mobile'));
        await runSlide(mobileSlides, mobileEl, 'slide-mobile-active');
    };
    ...
    const buildGoogleCalendar = () => {
        ...
        const data = new URLSearchParams({
            action: 'TEMPLATE',
            text: 'Lễ cưới Wahyu & Riski',
            dates: `${formatDate('2023-03-15 10:00')}/${formatDate('2023-03-15 11:00')}`,
            details: 'Kính mời Quý khách tới dự lễ cưới của chúng em. Xin chân thành cảm ơn tình cảm và lời chúc của Quý khách — đó là niềm vui và vinh dự lớn đối với chúng em.',
    ...
    /**
     * Trang trên GitHub Pages / domain thật không thể gọi API đặt tại localhost — bỏ qua bình luận để thiệp vẫn mở được.
     * @returns {boolean}
     */
    const canUseCommentApi = () => {
        const token = document.body.getAttribute('data-key') ?? '';
        if (!token || token.length <= 0) {
            return false;
        }

        const rawUrl = document.body.getAttribute('data-url') ?? '';
        if (!rawUrl) {
            return false;
        }
        ...
        let apiHost = '';
        try {
            apiHost = new URL(rawUrl, window.location.href).hostname;
        } catch {
            return false;
        }
    ...
    const pageLoaded = () => {
        ...
        const useCommentApi = canUseCommentApi();

        window.addEventListener('resize', util.debounce(slide));
        document.addEventListener('undangan.progress.done', () => startGuestUi());
        document.addEventListener('undangan.progress.invalid', () => startGuestUi());
