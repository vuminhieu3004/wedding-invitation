...
export const comment = (() => {
    ...
    const onNullComment = () => {
        const desc = lang
            .on('vi', '📢 Hãy chia sẻ thiệp để nhận thêm nhiều lời chúc nhé! 🎉')
            .get();
    ...
    const showOrHide = (button) => {
        ...
        button.innerText = isShow ? `Hiện phản hồi (${ids.length})` : 'Ẩn phản hồi';
    ...
    const showMore = (anchor, uuid) => {
        ...
        anchor.innerText = isCollapsed ? 'Thu gọn' : 'Xem đầy đủ';
    ...
    const update = async (button) => {
        ...
        if (!gifIsOpen && form.value?.trim().length === 0) {
            util.notify('Nội dung bình luận không được để trống.').warning();
            return;
        }
    ...
    const send = async (button) => {
        ...
        if (gifIsOpen && !gifId) {
            util.notify('Vui lòng chọn ảnh GIF.').warning();
            return;
        }
    ...
    const cancel = async (button, id) => {
        ...
        if (gif.isOpen(id) && ((!gif.getResultId(id) && isChecklist === isPresent) || util.ask('Bạn có chắc chắn?'))) {
            await gif.remove(id);
            removeInnerForm(id);
            return;
        }
        ...
        const form = document.getElementById(`form-inner-${id}`);
        if (form.value.length === 0 || (util.base64Encode(form.value) === form.getAttribute('data-original') && isChecklist === isPresent) || util.ask('Bạn có chắc chắn?')) {
            removeInnerForm(id);
            return;
        }
